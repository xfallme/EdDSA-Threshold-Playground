...
class ParticipantView:
    ...
    def round_one_commit(self, session_id: SessionId) -> None:
        self._coordinator_round_one(session_id, self._participant_id, self._PARTICIPANT.round_one_commit(session_id))
    ...
    def round_two_sign(self, session_id: SessionId) -> None:
        ...
        if signing_package is None:
            raise ValueError(f"Participant {self._participant_id} has not received a signing package for session {session_id}")
        self._coordinator_round_two(session_id, self._participant_id, self._PARTICIPANT.round_two_sign(signing_package))